...
class Object3D(object):
    ...
    def getDescendantsList(self):
        # master list of all descendants nodes
        descendants = []
        ...
        while len(nodesToProcess) > 0:
            ...
            # add this node to descendants list
    ...
    def applyMatrix(self, matrix, localCoordinate=True):
        if localCoordinate:
            self.transform = self.transform @ matrix
        else:
            self.transform = matrix @ self.transform

    def translate(self, x, y, z, localCoordinate=True):
        m = Matrix.makeTranslation(x, y, z)
        self.applyMatrix(m, localCoordinate)

    def rotateX(self, angle, localCoordinate=True):
        m = Matrix.makeRotationX(angle)
        self.applyMatrix(m, localCoordinate)

    def rotateY(self, angle, localCoordinate=True):
        m = Matrix.makeRotationY(angle)
        self.applyMatrix(m, localCoordinate)

    def rotateZ(self, angle, localCoordinate=True):
        m = Matrix.makeRotationZ(angle)
        self.applyMatrix(m, localCoordinate)

    def scale(self, s, localCoordinate=True):
    ...
    # return 3x3 submatrix with rotation data
